...
import { ResponsiveContainer, LineChart, Line, XAxis, YAxis, Tooltip, BarChart, CartesianGrid, Bar } from "recharts";
type propType = {
  earning: {
    today: number;
    sevenDays: number;
...
function AdminDashboardClient({ earning, stats, chartData }: propType) {
  const [filter, setFilter] = React.useState<"today" | "sevenDays" | "total">(
    "today",
  );
  ...
  return (
    <div className="pt-28 w-[90%] md:w-[80%] mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-10 text-center sm:text-left">
        ...
        <select
          className="border border-gray-700 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-green-500 outline-none transition w-full sm:w-auto"
          value={filter}
          onChange={(e) =>
            setFilter(e.target.value as "total" | "sevenDays" | "today")
          }
        >
        ...
        {stats.map((s, i) => {
          ...
          return (
            <motion.div
              ...
              className="bg-white bordr border-gray-100 shadow-md rounded-2xl p-5 flex items-center gap-4 hover:shadow-lg transition-all"
            >
              <div className="bg-green-200 p-3 rounded-xl">{icons[i]}</div>
              <div>
